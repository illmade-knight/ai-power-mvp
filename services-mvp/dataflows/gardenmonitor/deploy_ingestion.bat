...
gcloud run deploy "%SERVICE_NAME%" ^
  ...
  --port=8080 ^
  --set-env-vars="APP_PROJECT_ID=gemini-power-test,APP_PUBLISHER_TOPIC_ID=test-device-data,APP_BROKER_URL=tcp://broker.emqx.io:1883,APP_TOPIC=garden_monitor,APP_CLIENT_ID_PREFIX=garden_broker" ^
