...
func startBQProcessingService(t *testing.T, ctx context.Context, gcpProjectID, subID, datasetID, tableID string) (*bqstore.ProcessingService[types.GardenMonitorPayload], *bqinit.Server) {
	t.Helper()
	bqCfg := &bqinit.Config{
	...
	logger := log.With().Str("service", "bq-processor").Logger()
	bqClient, err := bigquery.NewClient(ctx, gcpProjectID)
	require.NoError(t, err)
	consumerConfig := &bqstore.GooglePubSubConsumerConfig{ProjectID: bqCfg.ProjectID, SubscriptionID: bqCfg.Consumer.SubscriptionID}
	consumer, err := bqstore.NewGooglePubSubConsumer(ctx, consumerConfig, logger)
	require.NoError(t, err)
	bqInserterConfig := &bqstore.BigQueryInserterConfig{ProjectID: bqCfg.ProjectID, DatasetID: bqCfg.BigQuery.DatasetID, TableID: bqCfg.BigQuery.TableID}
	inserter, err := bqstore.NewBigQueryInserter[types.GardenMonitorPayload](ctx, bqClient, bqInserterConfig, logger)
	require.NoError(t, err)
	batchInserterConfig := &bqstore.BatchInserterConfig{BatchSize: bqCfg.BatchProcessing.BatchSize, FlushTimeout: bqCfg.BatchProcessing.FlushTimeout}
	batcher := bqstore.NewBatchInserter[types.GardenMonitorPayload](batchInserterConfig, inserter, logger)
	serviceConfig := &bqstore.ServiceConfig{NumProcessingWorkers: bqCfg.BatchProcessing.NumWorkers}
	service, err := bqstore.NewBatchingService[types.GardenMonitorPayload](serviceConfig, consumer, batcher, types.GardenMonitorDecoder, logger)
	require.NoError(t, err)
	server := bqinit.NewServer(bqCfg, service, logger)
	go func() {
