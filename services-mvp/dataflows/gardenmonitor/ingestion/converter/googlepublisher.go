...
func (p *GooglePubSubPublisher) Publish(ctx context.Context, message *GardenMonitorMessage) error {
	if message == nil {
		return errors.New("cannot publish a nil message")
	}
	data, err := json.Marshal(message)
	if err != nil {
		p.logger.Error().Err(err).Str("device_eui", message.Payload.DE).Msg("Failed to marshal message for Pub/Sub")
