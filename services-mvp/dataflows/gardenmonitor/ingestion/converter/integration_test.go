...
func TestIngestionService_Integration_MQTT_To_PubSub(t *testing.T) {
	...
	t.Run("PublishAndReceiveMessage", func(t *testing.T) {
		...
		sourceMqttMsg := converter.GardenMonitorMessage{
			Topic:     testMqttTopicPattern,
			Timestamp: sampleMsgTime, // This field is part of the JSON for ParseMQTTMessage
		}
		...
		var receivedPayload converter.GardenMonitorMessage // Expecting the same structure
		...
		require.NotEmpty(t, receivedPayload.Payload, "Received Pub/Sub message is empty or EUI is missing")
		assert.Equal(t, testMqttDeviceEUI, receivedPayload.Payload.DE, "DeviceEUI mismatch")
		assert.Equal(t, payload, receivedPayload.Payload, "Payload mismatch")
		...
		assert.NotEmpty(t, receivedPayload.Payload.Sequence, "MessageID in received message should be populated by the service")

		// Compare timestamps (handle potential minor discrepancies if any)
		assert.WithinDuration(t, sampleMsgTime, receivedPayload.Timestamp, time.Second, "MessageTimestamp mismatch")
	})
