...
func TestEndToEndLoadScenario(t *testing.T) {
	...
	messengerDeviceGen, err := messenger.NewDeviceGenerator(messengerDeviceGenCfg, fsClientMessenger, loadGenLogger)
	require.NoError(t, err)
	...
	var allMessengerGeneratedDevices []*messenger.Device // Keep track of all devices messenger will try to publish for
