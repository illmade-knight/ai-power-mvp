...
func main() {
	...
	flag.StringVar(&cfg.GCPProjectID, "gcp-project", cfg.GCPProjectID, "GCP Project MessageID (required, can be set in YAML or CLI)")
	flag.StringVar(&cfg.EnrichedTopicID, "enriched-topic", cfg.EnrichedTopicID, "Short MessageID of the enriched Pub/Sub topic")
	flag.StringVar(&cfg.UnidentifiedTopicID, "unidentified-topic", cfg.UnidentifiedTopicID, "Short MessageID of the unidentified Pub/Sub topic")
	...
	flag.StringVar(&cfg.TestRunID, "run-id", "", "Unique MessageID for this test run (default: new UUID)") // Default to empty, will be set if not in YAML/CLI
