...
func (p *GooglePubSubPublisher) Publish(ctx context.Context, message *MQTTMessage) error {
	...
	if err != nil {
		p.logger.Error().Err(err).Str("device_eui", message.DeviceInfo.DeviceEUI).Msg("Failed to marshal EnrichedMessage for Pub/Sub")
		return fmt.Errorf("json.Marshal(EnrichedMessage): %w", err)
	}
	attributes := map[string]string{
		"message_type": "mqtt", // Add a type attribute
		//"mqtt_topic":   message.Topic(),
	}
	return p.publishData(ctx, data, attributes)
}

// publishData is a helper to reduce duplication.
func (p *GooglePubSubPublisher) publishData(ctx context.Context, data []byte, attributes map[string]string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("pubsub publish Get for %w", err)
	}
	p.logger.Debug().Str("message_id", msgID).Str("topic", p.topic.ID()).Interface("attributes", attributes).Msg("Message published successfully to Pub/Sub")
	return nil
}
