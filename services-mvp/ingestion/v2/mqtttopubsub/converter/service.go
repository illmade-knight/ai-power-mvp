...
func (s *IngestionService) processSingleMessage(ctx context.Context, msg InMessage, workerID int) {
	...
	if err := s.publisher.Publish(ctx, mqttMsg); err != nil {
		// This error would now likely be a setup or marshalling error, not a publish-wait error.
		s.logger.Error().
			Int("worker_id", workerID).
			Str("topic", mqttMsg.Topic).
			Str("eui", mqttMsg.DeviceInfo.DeviceEUI).
			Err(err).
			Msg("Failed to initiate publish for message")
		s.sendError(err)
	} else {
		s.logger.Debug().Int("worker_id", workerID).Str("device_eui", mqttMsg.DeviceInfo.DeviceEUI).Msg("Successfully processed and handed message to publisher")
	}
}
