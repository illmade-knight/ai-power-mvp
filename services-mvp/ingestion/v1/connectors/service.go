...
func (s *IngestionService) processSingleMessage(ctx context.Context, rawMsg []byte, workerID int) {
	...
	if err != nil {
		...
		s.logger.Info().Msg("Failed to enrich MQTT message")
		return
	}
...
func (s *IngestionService) Start() error {
	...
	if err := s.initAndConnectMQTTClient(); err != nil {
		...
		// Workers will exit as MessagesChan is not yet closed by Paho and they see context done.
		// Or they might process a few if any messages were already in MessagesChan (unlikely here).

		// Wait for workers to stop before fully exiting Start with error
		// We don't close MessagesChan here as Paho is not connected to write to it.
...
func (s *IngestionService) Stop() {
	...
	// s.logger.Info().Msg("Closing MessagesChan...")
	// close(s.MessagesChan) // Be cautious with this if Paho might still write.
	...
	if s.ErrorChan != nil { // Check if errorChan was initialized
		s.closeErrorChanOnce.Do(func() {
			s.logger.Info().Msg("Closing ErrorChan...")
			close(s.ErrorChan)
			s.logger.Info().Msg("ErrorChan closed.")
			// Optionally, you could set s.errorChan = nil here if you want to prevent
			// any further (even mistaken) attempts to use it, though sync.Once handles the close safety.
		})
	} else {
		s.logger.Info().Msg("ErrorChan is nil (was never initialized or already cleaned up), no need to close.")
	}
