...
func loadAppConfig() (*is.Config, error) { // Changed to return *is.Config
	...
	if cfg.ExpectedEnrichedTopicID == "" {
		return nil, fmt.Errorf("PUBSUB_TOPIC_ID_ENRICHED_MESSAGES environment variable for expected topic MessageID is required")
	}
	if cfg.ExpectedUnidentifiedTopicID == "" {
		return nil, fmt.Errorf("PUBSUB_TOPIC_ID_UNIDENTIFIED_MESSAGES environment variable for expected topic MessageID is required")
	}
