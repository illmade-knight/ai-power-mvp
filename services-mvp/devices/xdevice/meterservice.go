...
func (s *ProcessingService) processConsumedMessage(msg ConsumedMessage, workerID int) { // Updated receiver type
	...
	var upstreamMsg ConsumedUpstreamMessage
	...
	if upstreamMsg.ProcessingError != "" {
		s.logger.Warn().Str("msg_id", msg.ID).Str("device_eui", upstreamMsg.DeviceEUI).Str("upstream_error", upstreamMsg.ProcessingError).Msg("Received message with upstream processing error, Acking and skipping device-specific decoding.")
		msg.Ack()
		return
	}
	if upstreamMsg.RawPayload == "" {
		s.logger.Warn().Str("msg_id", msg.ID).Str("device_eui", upstreamMsg.DeviceEUI).Msg("Consumed message has empty RawPayload for device, Acking and skipping.")
		msg.Ack()
		return
	}

	// Decode the device-specific raw payload (e.g., for XDevice)
	// The function DecodePayload is assumed to be from the xdevice package (xdevice_decoder.go)
	decodedPayload, err := DecodePayload(upstreamMsg.RawPayload)
	if err != nil {
		s.logger.Error().Err(err).Str("msg_id", msg.ID).Str("device_eui", upstreamMsg.DeviceEUI).Str("raw_payload", upstreamMsg.RawPayload).Msg("Failed to decode device raw payload, Nacking.")
		msg.Nack()
		return
	}
